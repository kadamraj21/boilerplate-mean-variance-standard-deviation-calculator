...
def assertDictAlmostEqual(test_case, d1, d2, places=7):
    for key in d1:
        val1 = d1[key]
        val2 = d2[key]
            ...
            for sub1, sub2 in zip(val1, val2):
                if isinstance(sub1, list):
                    for x, y in zip(sub1, sub2):
                        test_case.assertAlmostEqual(x, y, places=places)
                else:
                    test_case.assertAlmostEqual(sub1, sub2, places=places)
        else:
            test_case.assertAlmostEqual(val1, val2, places=places)
...
class UnitTests(unittest.TestCase):
    def test_calculate(self):
        ...
        assertDictAlmostEqual(self, actual, expected)
